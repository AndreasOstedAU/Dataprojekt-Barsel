...
```{r}
r2 <- data.frame(label = c('R-squared: -0.0004787', 'R-squared: 0.08585', 'R-squared: 0.7013', 'R-squared: 0.8181', 'R-squared: 0.8782', 'R-squared: 0.8785'), 
...
plot_models <- ggplot(data = barsel04_models_long, mapping = aes(x = INDHOLD, y = Fitted, color=Model)) +
  ...
  facet_wrap(~Model) +
  geom_text(x = Inf, y = 15, aes(label = label), data = r2, inherit.aes = FALSE, size = 2, hjust = 1, vjust = 0) +
