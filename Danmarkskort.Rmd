...
barsel04_gns <- barsel04_big %>% filter(TAL %in% "Far - barselsdagpengedage i gennemsnit", 
                                ...
                                mutate(INDHOLD = parse_number(INDHOLD, locale = locale(decimal_mark = ",")),
                                       TID = factor(TID), OMRÅDE = str_remove_all(string = OMRÅDE, pattern = "Landsdel")) %>% group_by(OMRÅDE) %>% summarise(gns = mean(INDHOLD)) %>% arrange(desc(OMRÅDE))
barsel04_gns
```
```{r}
ggplotdata2 <- ggplotdata2 %>% mutate(gns_dage_land = barsel04_gns$gns)
ggplotdata2
```
```{r}
plot_dage <- ggplot(ggplotdata2$geometry, aes(fill = ggplotdata2$gns_dage_land)) +
  geom_sf() +
  theme_void() +
  labs(fill = "Gennemsnitligt antal barselsdage\n") +
  theme(
  ...
  scale_fill_gradient(low = 'grey94', high = 'navyblue')
...
plot_mk_land <- ggplot(ggplotdata2$geometry, aes(fill = ggplotdata2$Diff_køn_relativ)) +
  ...
  scale_fill_gradient2(low = 'firebrick1', mid = 'grey93', high = 'dodgerblue2',limit = c(-3, 3), labels = c('-3    (Flest kvinder)', '-2', '-1', '0', '1', '2', '3   (Flest mænd)')) +
...
plot_mk_komm <- ggplot(ggplotdata_komm, aes(fill = Relativ_forskel_mk)) +
  geom_sf() +
  theme_void() +
  scale_fill_gradient2(low = 'firebrick1', mid = 'grey93', high = 'dodgerblue2',limit = c(-5, 5), labels = c('-5    (Flest kvinder)', '-2.5', '0', '2.5', '5    (Flest mænd)')) +
  labs(fill = "Antal mænd - antal kvinder, \nforskel i procent\n") +
  theme(
    axis.text.x = element_blank(),
    axis.text.y = element_blank(),
...
plot_alder_m <- ggplot(ggplotdata_komm, aes(fill = `Gennemsnitsalder for førstegangs fædre`)) +
  ...
  theme(
    ...
    legend.position = c(1.075, 0.55),
    plot.margin = unit(c(0,3,0,0), 'cm')) + 
  scale_fill_gradient(low = 'grey96', high = 'dodgerblue4')
