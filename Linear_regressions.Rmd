...
```{r}
barsel_vj_nsj <- barsel04_far %>% filter(TID == 2019) %>% filter(OMRÅDE %in% c("Landsdel Vestjylland", "Landsdel Nordsjælland"))
barsel_vj_nsj
```

```{r}
pred <- predict(LM_interaction_far, newdata = barsel_vj_nsj)
pred
barsel_vj_nsj %>% mutate(Predikteret = pred) %>% mutate(Forskel = INDHOLD - Predikteret) %>% arrange(Forskel)
```
...
LM med ovenstående (broom til sammenligning). evt. plotte det på et dk kort. Lave rækkefølgen af landsdele om så de kommer vest -> øst georgrafisk på x-aksen.
