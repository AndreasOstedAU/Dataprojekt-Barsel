...
ggsave("Plots/Barselsdage_Uddannelse.png", plot=plot_barselsdage)
...
NordVestBrancher<-ford_branche %>% filter(OMRÅDE %in% c("Landsdel Vestjylland","Landsdel Nordsjælland")) %>%  
  ...
  ggplot(aes(x=Branche, y=`Andel i branche`*100, fill=OMRÅDE)) +
  geom_col(position = position_dodge2(),width = 0.8) +
  scale_fill_manual(values=c("palegreen2","skyblue2"),labels=c("Landsdel Vestjylland","Landsdel Nordsjælland"),name="Landsdel")+
  ylab("Procent")+
  xlab("Brancher")+
  theme_classic()+
  guides(fill=guide_legend(reverse=TRUE))+
